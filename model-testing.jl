...
infectionTime(τ) = infectionTimeStandardDeviation * normalRandom() + τ

function infect(p, τ)
  p.viralState = infectious
  p.infectionTime = infectionTime(τ)
...
function updateCity(Δt, population, buildings, roadMap, pointToCrossingDict, crossingMap, next, dist, roads, crossings, τ)
  infected = filter(isInfected, population)
  ...
  if length(Rvalues) == 0
    return length(infected), 0
  else
    return length(infected), mean(Rvalues)
  end
end
...
function simulate(τ, R = 1.15)
  local roads = createGraph(R)

  while !isConnected(roads)
    roads = createGraph(R)
  end

  local roadCount = length(roads)
  local crossings = getCrossings(roads)
  local pointToCrossingDict = getPointToCrossingDict(crossings)

  local dist, next = generatePathFindingMatrices(crossings, roads)

  local crossingMap = Dict{UInt,Int}(crossings[i].id => i for i ∈ 1:length(crossings))
  local roadMap = Dict{UInt,Road}(r.id => r for r in roads)
  ...
  for i in 1:frameCount
    data[i], Rvalues[i] = updateCity(1, population, buildings, roadMap, pointToCrossingDict, crossingMap, next, dist, roads, crossings, τ)
  end
...
f1 = open("output-4a.csv", "w")
f2 = open("output-4b.csv", "w")

for k in 1:length(τ)
  lock(lk0) do
    println(k, '/', length(τ), " started")
  end
